...
TEST( VariadicLoggerTest, Initialization )
{
  VariadicLogger::Pointer variadicLogger = VariadicLogger::New();
}
...
TEST( VariadicLoggerTest, Console )
{
  ...
  logger->SetLogLevel( VariadicLogger::LogLevel::trace );
  logger->Trace( "Console TRACE message" );
  logger->Debug( "Console DEBUG message" );

  logger->Info( "Console INFO message with pattern {0:d}", 42 );
}
